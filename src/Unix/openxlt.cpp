...
time_t serialConvert(long double s) { /* convert day number to y,m,d format */
    time_t rawtime;
...
int getSampleNumberFromETC(string path, string filename) {
    int sampleNumber;
    //Load ETC file
    FILE *etcf = fopen((path + filename + string(".etc")).c_str(), "r");
...
int main(int argc, const char * argv[])
{
    ...
    string path(argv[1]);
    string name(basename(const_cast<char*>(argv[1])));
    cout << (path + name) << endl;
    FILE *eegf = fopen((path + name + string(".eeg")).c_str(), "r");
    if(!eegf) {
    ...
    regmatch_t match[2];
    int reti;
    reti = regcomp(&regex, "\\(\\.\"SamplingFreq\", ([0-9]+\\.[0-9]+)\\)", REG_EXTENDED);
    reti = regexec(&regex, eegcontents, 3, match, REG_NOTBOL);
    char *sambuf = new char[match[1].rm_eo-match[1].rm_so];
    strncpy(sambuf, eegcontents+match[1].rm_so, match[1].rm_eo-match[1].rm_so);
    samplingFrequency = atoi(sambuf);
    regfree(&regex);
    
    reti = regcomp(&regex, "\\(\\.\"CreationTime\", ([0-9]+\\.[0-9]+)\\)", REG_EXTENDED);
    reti = regexec(&regex, eegcontents, 3, match, REG_NOTBOL);
    sambuf = new char[match[1].rm_eo-match[1].rm_so];
    strncpy(sambuf, eegcontents+match[1].rm_so, match[1].rm_eo-match[1].rm_so);
    creationSeconds = serialConvert(atof(sambuf));
    regfree(&regex);
    
    //Repeatedly parse STC file for duration and file name data
    FILE *stcf = fopen((path + name + string(".stc")).c_str(), "r");
    if(!stcf) {
    ...
    while ((f = fgetc(stcf)) != EOF) {
        ...
        //Load ERD file
        FILE *erdf = fopen((path + filename + string(".erd")).c_str(), "r");
